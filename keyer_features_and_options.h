...
// The ESP32 does not seem to suffer from this problem so might be a better option if faithful Winkey emulation
// is needed (e.g. for Iambic Master)

// Command Line Interface functionality - JBA was disabled
// Enabling this causes a whole bunch of spaces to be sent to the serial port and clogs things up
// The problem appears to be in service_paddle_echo - see comments there
// You can fix the problem by pressing the paddles while its starting up.
//  At this point, I'm not sure why I wanted this feature anyway so its desabled again.
...
#define FEATURE_SEPARATE_PADDLE_SPEED           // JBA new feature to use separate speeds for computer and paddles
